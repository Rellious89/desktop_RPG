...
PerfectPixel에 실제로 전달하는 텍스트는 두 개뿐이다.
...
| Character description | 1~2개의 짧은 영문 문장. 외형·장비·승인 시점만 기록 |
| Art style | `Pixel Art` |
...
| Facing direction | 기준 이미지의 시점을 보존해야 할 때 `Not set`을 우선 시험. 방향 지정 시 시점이 변하는지 출력으로 검증 |
...
- 컨셉 이미지는 48/64px 논리 해상도로 미리 축소하지 않는다. PerfectPixel이 자체 픽셀 밀도를 다시 만들기 때문에 가장 선명한 원본을 입력한다.
- 캐릭터 간 상대 체격은 입력 패딩으로 통제하지 않는다. 출력 후 캐릭터별 표시 배율을 별도 기록하고 Unity 화면에서 확정한다.
- 최종 논리 픽셀 밀도는 아직 고정하지 않는다. 512 출력의 선명도와 실제 게임 표시 결과를 먼저 비교한다.
