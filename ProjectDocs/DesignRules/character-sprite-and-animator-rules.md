...
모든 신규 캐릭터·몬스터와 기존 Actor의 재설계·애니메이션 추가는
[`character-production-package-rules.md`](./character-production-package-rules.md)에 따라 Actor별 제작 패키지에
기록한다. 이 문서의 512×512 / PPU 200 값은 1차 V1 생산 기준이다. 다른 검증 프로필을 사용하는 Actor는
패키지에 Profile 이름과 캔버스·PPU·Pivot을 명시하며, 이를 전체 공통값으로 승격하기 전에는 기존 규격과
자동 혼용하지 않는다.
...
- 플레이어 1종: 기본 Idle 4~6프레임, Idle 변형 1종 이상, 공격 A 1종과 공격 B 1종. Tier 1 풀은 A,
  Tier 2 풀은 A+B로 구성한다. 최종 출시 목표는 공격 C를 더한 Tier 3 풀(A+B+C)이지만 현재 버티컬
  슬라이스의 필수 범위는 아니다.
