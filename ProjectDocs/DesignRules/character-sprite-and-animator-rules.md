...
| 체격 차이에 따른 크기 표현 | 모든 캐릭터는 512×512 캔버스를 공통으로 쓰되, PerfectPixel 입력 패딩으로 상대 체격을 통제하지 않는다. 각 Actor는 `relativeVisualScale`에 해당하는 목표 비율을 별도로 기록하고 출력 후 Unity 화면에서 확정한다. 실제 구현은 전용 VisualRoot Scale 또는 원화 공통 리사이즈 중 품질이 좋은 쪽을 버티컬 슬라이스에서 선택한다 | PerfectPixel이 캐릭터 콘텐츠를 캔버스에 맞게 다시 확대하므로 원화 점유율만으로 체격을 보존할 수 없다. 캐릭터 간 체격과 같은 Actor 내부의 프레임별 축척 문제를 분리해야 한다 |
| 공통 시각 밀도 | **`Low Companion v1`**: 승인된 바바리안 최종 스프라이트와 동일한 굵은 저밀도 픽셀 덩어리(후가공 기준 약 3×3)를 사용한다. 고밀도/미세 묘사 Master는 현재 생산 범위에서 제외한다 | FireAlpaca에서 프레임별 후보정이 가능한 생산성, 실제 데스크톱 컴패니언 화면에서의 가독성을 함께 검증해 선택한 기준 |
...
- **캐릭터별 상대 표시 배율 구현** — 입력 패딩이 무시되는 것이 확인되어 캐릭터별 목표 체격값은 별도로
  관리한다. 고밀도 BlackCatMage 비교에서 `VisualRoot Scale 0.35`는 실험에는 유효했지만 생산용 해결책으로
  채택하지 않았다. Low Companion v1 원화를 Unity 기본 Scale 1에서 맞추는 공정을 우선 검증한다.
