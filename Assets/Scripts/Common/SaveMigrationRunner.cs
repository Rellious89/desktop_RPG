...
namespace Common
{
    ...
    /// 없는 캐릭터는 가지고 있지 않은 캐릭터다. v1에는 그 뜻이 없었다: <b>쓸 수 있는 캐릭터를 정한 것은
    /// 씬에 직렬화된 로스터 목록</b>이었고, 저장 목록은 그 목록을 따라 만들어진 <b>상태 기록</b>이었다
    /// (CharacterRoster.Awake가 쓸 수 있는 모든 항목에 대해 상태를 만들어 두었다). 그래서 저장 목록의
    /// 유무는 보유를 뜻하지 않았다 - 로스터에서 빼면 항목이 남아 있어도 쓸 수 없었고, 로스터에 넣으면
    /// 항목이 없어도 다음 실행에 생겼다.
    ///
    /// v2는 그 권한을 저장 문서로 옮긴다. 그래서 v1 문서를 그대로 v2의 규칙으로 읽으면 <b>그 문서가
    /// 저장될 당시 아직 항목이 만들어지지 않았던 캐릭터</b>(그 캐릭터가 로스터에 들어오기 전에 저장된
    /// 문서 등)가 미보유가 되어 가진 것이 줄어든다. 이 단계가 그 여섯을 보유로 확정해 그 일을 막는다.
    ///
