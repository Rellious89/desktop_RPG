    ...
    public static class CharacterStoryQuestUiPrefabSetup
    {
        ...
        public static void Setup()
        {
            ...
            try
            {
                ...
                if (allListItem == null) throw new System.InvalidOperationException("전체 퀘스트 목록 템플릿 컴포넌트를 찾을 수 없습니다.");
                SerializedObject allListItemSerialized = new SerializedObject(allListItem);
                Set(allListItemSerialized, "titleText", allListTitle.GetComponent<TMP_Text>());
                Set(allListItemSerialized, "questTypeText", Find(allListTemplate, "lb_QuestType").GetComponent<TMP_Text>());
                Set(allListItemSerialized, "titleLocalizer", allListTitle.GetComponent<LocalizedTMPText>());
                Set(allListItemSerialized, "completeLabel", allListComplete.gameObject);
                allListItemSerialized.ApplyModifiedPropertiesWithoutUndo();
                Set(serialized, "allQuestListRoot", allList.gameObject);
                Set(serialized, "allQuestListScroll", allListScroll.GetComponent<ScrollRect>());
                Set(serialized, "allQuestListContent", allListContent.GetComponent<RectTransform>());
                Set(serialized, "allQuestListItemTemplate", allListItem);
                Set(serialized, "allQuestSelection", Find(allListScroll, "Viewport/sp_selectAni").GetComponent<RectTransform>());
